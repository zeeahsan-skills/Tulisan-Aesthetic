...
interface FacebookFontStylesProps {
  onCopy: (msg: string) => void;
}

export function FacebookFontStyles({ onCopy }: FacebookFontStylesProps) {
  const [copiedId, setCopiedId] = useState<string | null>(null);
  const [favorites, setFavorites] = useState<Record<string, boolean>>({});

  const sampleText = 'Facebook Aesthetic Profile 📘';

  const handleCopy = (transformedText: string, styleId: string) => {
    navigator.clipboard.writeText(transformedText);
    setCopiedId(styleId);
    onCopy('Font disalin ke clipboard!');
  ...
  const handleShare = (text: string, styleName: string) => {
    if (typeof navigator !== 'undefined' && navigator.share) {
      navigator
        .share({
          title: `Facebook Style: ${styleName}`,
          text: text,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto">
          <span className="px-3.5 py-1.5 rounded-full bg-blue-500/10 border border-blue-500/20 text-xs font-bold uppercase tracking-wider text-blue-600 dark:text-blue-400">
            Katalog Variasi Font
          </span>
          <h2 className="mt-4 text-3xl sm:text-5xl font-extrabold text-slate-900 dark:text-white font-poppins">
            Koleksi Gaya Font Facebook Unicode
          </h2>
          <p className="mt-4 text-base sm:text-lg text-slate-600 dark:text-slate-400">
            Pilih variasi font tulisan tangan, tebal sans-serif, miring cursive, bubble, hingga gothic untuk menyempurnakan status Facebook Anda.
          </p>
        </div>
          ...
          {ALL_FONT_STYLES.map((style, idx) => {
            ...
            return (
              <motion.div
                ...
                className="group relative rounded-3xl p-6 bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-blue-900/20 hover:border-blue-500/50 shadow-md hover:shadow-xl transition-all duration-300 flex flex-col justify-between"
              >
                <div>
                ...
                <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                  <span className="text-xs text-slate-500 dark:text-slate-400">100% FB Compatible</span>
                  <button
                  ...
                  >
                    ...
                    <span>{isCopied ? 'Tersalin!' : 'Salin Style'}</span>
                  </button>
                </div>
              </motion.div>
            );
