...
interface DiscordFontStylesProps {
  onCopy: (msg: string) => void;
}

export function DiscordFontStyles({ onCopy }: DiscordFontStylesProps) {
  const [copiedId, setCopiedId] = useState<string | null>(null);
  const [favorites, setFavorites] = useState<Record<string, boolean>>({});

  const sampleText = 'Discord Server VIP 🎮';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto">
          ...
          <h2 className="mt-4 text-3xl sm:text-5xl font-extrabold text-slate-900 dark:text-white font-poppins">
            Koleksi Gaya Font Discord Unicode
          </h2>
          <p className="mt-4 text-base sm:text-lg text-slate-600 dark:text-slate-400">
            Pilih variasi font Gothic, Small Caps, Cyber Glitch, Cursive miring, hingga Tebal Sans-serif untuk server & profil Discord Anda.
          </p>
        </div>
          ...
          {ALL_FONT_STYLES.map((style, idx) => {
            ...
            return (
              <motion.div
                ...
                className="group relative rounded-3xl p-6 bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-indigo-900/20 hover:border-indigo-500/50 shadow-md hover:shadow-xl transition-all duration-300 flex flex-col justify-between"
              >
                <div>
                ...
                <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                  <span className="text-xs text-slate-500 dark:text-slate-400">100% Discord Ready</span>
                  <button
