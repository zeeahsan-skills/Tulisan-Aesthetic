...
interface InstagramFontStylesProps {
  onCopy: (msg: string) => void;
}

export function InstagramFontStyles({ onCopy }: InstagramFontStylesProps) {
  const [copiedId, setCopiedId] = useState<string | null>(null);
  const [favorites, setFavorites] = useState<Record<string, boolean>>({});

  const sampleText = 'Instagram Bio Aesthetic';

  const handleCopySingle = (transformedText: string, styleId: string) => {
    navigator.clipboard.writeText(transformedText);
    setCopiedId(styleId);
    onCopy('Teks Instagram berhasil disalin!');
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto">
          ...
          <p className="mt-4 text-base sm:text-lg text-slate-600 dark:text-slate-400">
            Koleksi gaya huruf Unicode aesthetic terlengkap untuk mempercantik bio, caption, dan highlight profil Instagram Anda.
          </p>
        </div>
          ...
          {ALL_FONT_STYLES.slice(0, 18).map((style, idx) => {
            ...
            return (
              <motion.div
                ...
                className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-purple-900/30 shadow-lg hover:shadow-2xl hover:border-pink-500/30 transition-all flex flex-col justify-between gap-4 group"
              >
                <div>
                ...
                <button
                  ...
                  className={`w-full inline-flex items-center justify-center gap-2 py-3 rounded-xl text-xs font-semibold transition-all ${
                    isCopied
                      ...
                      : 'bg-gradient-to-r from-purple-600 via-pink-500 to-rose-500 text-white hover:opacity-95 shadow-md shadow-pink-500/15'
                  }`}
                >
                  {isCopied ? (
                    <>
