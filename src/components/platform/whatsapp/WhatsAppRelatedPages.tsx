...
import { ArrowRight, Type, Bold, Italic, User, Camera, Video } from 'lucide-react';

export function WhatsAppRelatedPages() {
  const pages = [
    {
    ...
    },
    {
      ...
      desc: 'Ubah teks bio, nama tampilan & caption video TikTok menjadi estetik.',
          ...
          {pages.map((item, idx) => {
            ...
            return (
              ...
              >
                <Link
                  href={item.href}
                  className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-emerald-900/30 shadow-md hover:shadow-xl hover:border-emerald-500/40 transition-all flex flex-col justify-between h-full group"
                >
                  <div>
                    ...
                    <h3 className="text-lg font-bold text-slate-900 dark:text-white font-poppins group-hover:text-emerald-500 transition-colors">
                      {item.title}
                    </h3>
                  ...
                  <div className="mt-4 flex items-center gap-1.5 text-xs font-semibold text-emerald-600 dark:text-emerald-400 group-hover:translate-x-1 transition-transform">
