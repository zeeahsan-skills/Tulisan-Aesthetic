...
import { Copy, Shuffle, Trash2, Check, Heart, Share2, Target } from 'lucide-react';
...
export function PubgHero({ onCopy }: PubgHeroProps) {
  const [inputText, setInputText] = useState('CONQUEROR 🎯');
  ...
  const handleCopySingle = (transformedText: string, styleId: string) => {
    ...
    setCopiedId(styleId);
    onCopy('Nickname PUBG Mobile berhasil disalin!');
    setTimeout(() => setCopiedId(null), 2000);
  };

  const handleCopyAll = () => {
    const allFormatted = ALL_FONT_STYLES.slice(0, 10)
      .map((s) => `乂${s.transform(inputText || 'CONQUEROR')}乂`)
      .join('\n\n');
    navigator.clipboard.writeText(allFormatted);
    onCopy('Semua nickname PUBG Mobile berhasil disalin!');
  };
  ...
  const handleShare = (text: string, title: string) => {
    if (typeof navigator !== 'undefined' && navigator.share) {
      navigator
        .share({
          title: `PUBG Mobile Name: ${title}`,
          text: text,
          url: window.location.href,
        })
        .catch(() => {});
    } else {
      navigator.clipboard.writeText(`${title}: ${text}\nGenerated via https://tulisanaesthetic.id/game/pubg-mobile`);
  ...
  return (
    <section id="hero" className="relative pt-12 pb-20 md:pt-20 md:pb-32 overflow-hidden">
      ...
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-gradient-to-tr from-yellow-600/20 via-amber-500/20 to-orange-600/10 rounded-full blur-3xl -z-10 pointer-events-none" />
      <div className="absolute top-1/3 right-10 w-96 h-96 bg-amber-500/15 rounded-full blur-3xl -z-10 pointer-events-none" />
      <div className="absolute bottom-10 left-10 w-96 h-96 bg-yellow-500/15 rounded-full blur-3xl -z-10 pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-yellow-500/10 via-amber-500/10 to-orange-500/10 border border-yellow-500/20 text-xs sm:text-sm font-semibold text-amber-700 dark:text-amber-300 backdrop-blur-md shadow-sm">
            <Target className="w-4 h-4 text-amber-500" />
            <span>PUBG Mobile Stylish Name Generator 2026</span>
          </span>
        </motion.div>
        ...
        >
          <h1 className="text-4xl sm:text-6xl lg:text-7xl font-extrabold tracking-tight text-slate-900 dark:text-white font-poppins leading-[1.15]">
            ...
            <span className="bg-gradient-to-r from-yellow-500 via-amber-500 to-orange-500 bg-clip-text text-transparent">
              Generator Nama PUBG Keren
            </span>
          </h1>
          <p className="mt-6 text-lg sm:text-xl text-slate-600 dark:text-slate-300 max-w-2xl mx-auto leading-relaxed">
            Buat nickname PUBG Mobile estetik bergaya Conqueror & Clan Pro dengan kombinasi simbol Jepang (乂, 帝, 乡), mahkota 👑, tag clan, dan font Unicode secara instan.
          </p>
        </motion.div>
        ...
        >
          <div className="relative">
            <textarea
              ...
              placeholder="Tulis nickname PUBG Mobile di sini... (contoh: Conqueror)"
              rows={3}
          ...
          <div className="mt-5 flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-slate-100 dark:border-slate-800">
            ...
            <button
              ...
              className="w-full sm:w-auto px-6 py-3 rounded-xl bg-gradient-to-r from-yellow-500 to-amber-600 hover:from-yellow-600 hover:to-amber-700 text-white font-bold text-sm shadow-lg shadow-amber-500/25 flex items-center justify-center gap-2 transition-all hover:scale-[1.02]"
            >
              <Copy className="w-4 h-4" />
              <span>Salin Semua Nickname PUBG</span>
            </button>
          </div>
        </motion.div>
        ...
        <div className="mt-12 max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          {ALL_FONT_STYLES.slice(0, 8).map((style) => {
            const baseTransformed = style.transform(inputText || 'CONQUEROR');
            const transformed = `乂${baseTransformed}乂`;
            ...
            return (
              ...
              >
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                  <span className="text-[11px] text-slate-400">100% PUBG Mobile Compatible</span>
                  <button
