...
interface PubgFontStylesProps {
  onCopy: (msg: string) => void;
}

export function PubgFontStyles({ onCopy }: PubgFontStylesProps) {
  const [copiedId, setCopiedId] = useState<string | null>(null);
  const [favorites, setFavorites] = useState<Record<string, boolean>>({});

  const sampleText = 'PUBG MOBILE 🎯';

  const handleCopy = (transformedText: string, styleId: string) => {
    navigator.clipboard.writeText(transformedText);
    setCopiedId(styleId);
    onCopy('Nickname PUBG Mobile disalin ke clipboard!');
  ...
  const handleShare = (text: string, styleName: string) => {
    if (typeof navigator !== 'undefined' && navigator.share) {
      navigator
        .share({
          title: `PUBG Mobile Name: ${styleName}`,
          text: text,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto">
          <span className="px-3.5 py-1.5 rounded-full bg-amber-500/10 border border-amber-500/20 text-xs font-bold uppercase tracking-wider text-amber-600 dark:text-amber-400">
            Katalog Nickname PUBG
          </span>
          <h2 className="mt-4 text-3xl sm:text-5xl font-extrabold text-slate-900 dark:text-white font-poppins">
            Koleksi Style Nickname PUBG Mobile
          </h2>
          <p className="mt-4 text-base sm:text-lg text-slate-600 dark:text-slate-400">
            Pilih variasi simbol Jepang (乂, 帝, 乡), mahkota, font Small Caps, Gothic Fraktur, hingga spasi transparan PUBG.
          </p>
        </div>

        <div className="mt-14 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {ALL_FONT_STYLES.map((style, idx) => {
            const transformed = `乂${style.transform(sampleText)}乂`;
            ...
            return (
              ...
              >
                ...
                <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                  <span className="text-xs text-slate-500 dark:text-slate-400">100% PUBG Mobile Ready</span>
                  <button
