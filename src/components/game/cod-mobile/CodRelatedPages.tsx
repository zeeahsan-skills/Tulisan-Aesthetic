...
import { ArrowRight, Type, Flame, Crosshair, Swords, Gamepad2, MessageSquare, Sparkles } from 'lucide-react';

export const CodRelatedPages: React.FC = () => {
  const links = [
    {
    ...
    },
    {
      ...
      path: '/game/roblox',
      desc: 'Username & Display Name Roblox estetik.',
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {links.map((item, idx) => (
            ...
            >
              <Link
                href={item.path}
                className="bg-slate-900/80 border border-slate-800 hover:border-orange-500/50 rounded-2xl p-5 shadow-lg flex flex-col justify-between group transition-all block h-full hover:-translate-y-1"
              >
                <div>
                  ...
                  <h3 className="text-base font-bold text-slate-100 group-hover:text-orange-300 transition-colors mb-1.5">
                    {item.title}
                  </h3>
