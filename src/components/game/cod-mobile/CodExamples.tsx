...
import { COD_POPULAR_EXAMPLES } from '@/lib/cod-mobile-presets';

interface CodExamplesProps {
  onCopy: (msg: string) => void;
}

export const CodExamples: React.FC<CodExamplesProps> = ({ onCopy }) => {
  const [copiedIndex, setCopiedIndex] = useState<number | null>(null);

  const handleCopy = (styledName: string, idx: number) => {
    navigator.clipboard.writeText(styledName);
    setCopiedIndex(idx);
    onCopy(`Nickname "${styledName}" berhasil disalin!`);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          <h2 className="text-2xl sm:text-4xl font-extrabold mb-3">
            Popular COD Mobile Nickname Examples
          </h2>
          <p className="text-slate-400 text-sm sm:text-base">
            Kumpulan preset nickname terpopuler untuk player Call of Duty: Mobile. Ketuk tombol <strong className="text-orange-400">Copy</strong> di sebelah kanan untuk langsung menyalin.
          </p>
        </div>

        {/* Examples Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4 sm:gap-6">
          {COD_POPULAR_EXAMPLES.map((item, idx) => {
            ...
            return (
              <motion.div
                ...
                className="bg-slate-900/90 border border-slate-800/90 hover:border-orange-500/50 rounded-2xl p-5 shadow-lg flex flex-col justify-between group transition-all"
              >
                <div>
                  ...
                  <div className="my-3 p-3.5 bg-slate-950 rounded-xl border border-slate-800/90 flex items-center justify-between gap-3">
                    <span className="text-lg sm:text-xl font-bold font-mono text-amber-200 truncate select-all">
                      {item.styledName}
                    </span>
