...
interface CodFontStylesProps {
  onCopy: (msg: string) => void;
}

export const CodFontStyles: React.FC<CodFontStylesProps> = ({ onCopy }) => {
  const [baseText, setBaseText] = useState('COD Mobile');
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem('codm_fav_fonts');
  ...
  const toggleFavorite = (styleId: string) => {
    ...
    try {
      localStorage.setItem('codm_fav_fonts', JSON.stringify(updated));
    } catch {
      // Ignore
    }
  };
  ...
  const handleCopy = (text: string, id: string) => {
    ...
    setCopiedId(id);
    onCopy('Variasi nama CODM berhasil disalin!');
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'COD Mobile Stylish Name',
          text: `Cek nickname CODM estetik ini: ${text}`,
      ...
      navigator.clipboard.writeText(text);
      onCopy('Link & nickname disalin ke clipboard!');
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          <h2 className="text-2xl sm:text-4xl font-extrabold mb-3">
            COD Mobile Name Styles Catalog
          </h2>
          <p className="text-slate-400 text-sm sm:text-base">
            Pilih variasi font Unicode estetik di bawah ini. Ketuk tombol <strong className="text-orange-300">Copy</strong>, <strong className="text-pink-400">Favorite</strong>, atau <strong className="text-cyan-400">Share</strong> untuk langsung digunakan pada akun CODM Anda.
          </p>
          ...
          <div className="mt-6 max-w-md mx-auto">
            <input
              ...
              className="w-full bg-slate-900 border border-slate-700 focus:border-orange-500 rounded-xl px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-orange-500/20 text-center"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4 sm:gap-6">
          {displayStyles.map((style, idx) => {
            const transformed = style.transform(baseText.trim() || 'COD Mobile');
            const decorated = `💀${transformed}💀`;
            ...
            return (
              <motion.div
                ...
                className="bg-slate-900/90 border border-slate-800 hover:border-orange-500/50 rounded-2xl p-5 shadow-lg hover:shadow-orange-500/10 transition-all flex flex-col justify-between group"
              >
                {/* Card Top Header */}
        ...
        {previewItem && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
            <motion.div
              ...
              className="bg-slate-900 border border-orange-500/40 rounded-2xl p-6 max-w-md w-full shadow-2xl text-center text-white relative"
            >
              <h3 className="text-lg font-bold mb-1 text-orange-300">{previewItem.label} Preview</h3>
              <p className="text-xs text-slate-400 mb-4">Tampilan Nickname di In-Game COD Mobile</p>
              
              <div className="bg-slate-950 p-4 rounded-xl border border-orange-500/30 text-xl font-mono break-all text-amber-300 mb-6 shadow-inner">
                {previewItem.text}
              </div>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 py-2.5 bg-orange-600 hover:bg-orange-500 text-white font-semibold rounded-xl text-sm transition-all"
                >
                  Copy Nickname
                </button>
                <button
