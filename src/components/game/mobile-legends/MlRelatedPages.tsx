...
import { ArrowRight, Type, Flame, Crosshair, Gamepad2, MessageSquare, Sparkles } from 'lucide-react';

export const MlRelatedPages: React.FC = () => {
  const links = [
    {
    ...
    },
    {
      ...
      path: '/roblox',
      desc: 'Font & username estetik khusus player Roblox.',
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {links.map((item, idx) => (
            ...
            >
              <Link
                href={item.path}
                className="bg-slate-900/80 border border-slate-800 hover:border-indigo-500/50 rounded-2xl p-5 shadow-lg flex flex-col justify-between group transition-all block h-full hover:-translate-y-1"
              >
                <div>
                  ...
                  <h3 className="text-base font-bold text-slate-100 group-hover:text-indigo-300 transition-colors mb-1.5">
                    {item.title}
                  </h3>
