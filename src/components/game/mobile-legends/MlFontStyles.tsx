...
interface MlFontStylesProps {
  onCopy: (msg: string) => void;
}

export const MlFontStyles: React.FC<MlFontStylesProps> = ({ onCopy }) => {
  const [baseText, setBaseText] = useState('Mobile Legends');
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem('mlbb_fav_fonts');
  ...
  const toggleFavorite = (styleId: string) => {
    ...
    try {
      localStorage.setItem('mlbb_fav_fonts', JSON.stringify(updated));
    } catch {
      // Ignore
    }
  };
  ...
  const handleCopy = (text: string, id: string) => {
    ...
    setCopiedId(id);
    onCopy('Variasi nama berhasil disalin!');
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'Mobile Legends Stylish Name',
          text: `Cek nickname MLBB estetik ini: ${text}`,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          <h2 className="text-2xl sm:text-4xl font-extrabold mb-3">
            Stylish Mobile Legends Names Catalog
          </h2>
          <p className="text-slate-400 text-sm sm:text-base">
            Pilih variasi font Unicode estetik di bawah ini. Ketuk tombol <strong className="text-indigo-300">Copy</strong>, <strong className="text-pink-400">Favorite</strong>, atau <strong className="text-cyan-400">Share</strong> untuk langsung digunakan pada akun MLBB Anda.
          </p>
          ...
          <div className="mt-6 max-w-md mx-auto">
            <input
              ...
              className="w-full bg-slate-900 border border-slate-700 focus:border-indigo-500 rounded-xl px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 text-center"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4 sm:gap-6">
          {displayStyles.map((style, idx) => {
            const transformed = style.transform(baseText.trim() || 'Mobile Legends');
            ...
            return (
              <motion.div
                ...
                className="bg-slate-900/90 border border-slate-800 hover:border-indigo-500/50 rounded-2xl p-5 shadow-lg hover:shadow-indigo-500/10 transition-all flex flex-col justify-between group"
              >
                {/* Card Top Header */}
        ...
        {previewItem && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
            <motion.div
              ...
              className="bg-slate-900 border border-indigo-500/40 rounded-2xl p-6 max-w-md w-full shadow-2xl text-center text-white relative"
            >
              <h3 className="text-lg font-bold mb-1 text-indigo-300">{previewItem.label} Preview</h3>
              <p className="text-xs text-slate-400 mb-4">Tampilan Nickname di In-Game Mobile Legends</p>
              
              <div className="bg-slate-950 p-4 rounded-xl border border-indigo-500/30 text-xl font-mono break-all text-amber-300 mb-6 shadow-inner">
                {previewItem.text}
              </div>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold rounded-xl text-sm transition-all"
                >
                  Copy Nickname
                </button>
                <button
