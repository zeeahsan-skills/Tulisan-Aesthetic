...
import { COOL_NAMES_ARTICLE_META } from '@/lib/cool-names-article';
import { Clock, User, Calendar, Share2, Sparkles, ChevronRight } from 'lucide-react';

interface CoolNamesHeroHeaderProps {
  onCopy: (msg: string) => void;
}

export const CoolNamesHeroHeader: React.FC<CoolNamesHeroHeaderProps> = ({ onCopy }) => {
  const handleShare = async () => {
    if (navigator.share) {
      try {
        await navigator.share({
          title: COOL_NAMES_ARTICLE_META.title,
          text: COOL_NAMES_ARTICLE_META.subtitle,
    ...
    <section className="pt-10 pb-12 bg-gradient-to-b from-slate-900 via-purple-950/30 to-slate-950 text-white relative border-b border-slate-800">
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        ...
        >
          ...
          <span>{COOL_NAMES_ARTICLE_META.category}</span>
        </motion.div>
        ...
        >
          {COOL_NAMES_ARTICLE_META.subtitle}
        </motion.p>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-slate-800/80 text-xs text-slate-400">
          <div className="flex items-center gap-4 flex-wrap">
            <span className="flex items-center gap-1.5 font-semibold text-slate-200">
              ...
              {COOL_NAMES_ARTICLE_META.author}
            </span>
            <span className="flex items-center gap-1">
              <Calendar className="w-3.5 h-3.5 text-slate-500" />
              {COOL_NAMES_ARTICLE_META.publishDate}
            </span>
            <span className="flex items-center gap-1">
              <Clock className="w-3.5 h-3.5 text-slate-500" />
              {COOL_NAMES_ARTICLE_META.readingTime}
            </span>
          </div>
