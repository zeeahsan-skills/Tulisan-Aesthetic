...
import { TIKTOK_ARTICLE_META } from '@/lib/tiktok-article';
import { Clock, User, Calendar, Share2, Sparkles, ChevronRight, Video } from 'lucide-react';

interface TikTokHeroHeaderProps {
  onCopy: (msg: string) => void;
}

export const TikTokHeroHeader: React.FC<TikTokHeroHeaderProps> = ({ onCopy }) => {
  const handleShare = async () => {
    if (navigator.share) {
      try {
        await navigator.share({
          title: TIKTOK_ARTICLE_META.title,
          text: TIKTOK_ARTICLE_META.subtitle,
  ...
  return (
    <section className="pt-10 pb-12 bg-gradient-to-b from-slate-900 via-slate-900/90 to-slate-950 text-white relative border-b border-slate-800">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-4xl h-64 bg-cyan-500/10 blur-[100px] pointer-events-none rounded-full" />
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        <motion.div
          ...
          className="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full bg-cyan-500/10 border border-cyan-500/30 text-cyan-300 text-xs font-semibold mb-4"
        >
          <Video className="w-3.5 h-3.5 text-cyan-400" />
          <span>{TIKTOK_ARTICLE_META.category}</span>
        </motion.div>
        ...
        <motion.h1
          ...
          className="text-3xl sm:text-5xl font-extrabold tracking-tight mb-4 leading-tight bg-clip-text text-transparent bg-gradient-to-r from-cyan-200 via-pink-200 to-purple-200"
        >
          {TIKTOK_ARTICLE_META.title}
        </motion.h1>
