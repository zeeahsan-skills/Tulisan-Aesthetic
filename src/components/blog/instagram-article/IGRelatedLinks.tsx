...
import { ArrowRight, BookOpen, Type, Sparkles, MessageCircle, Camera } from 'lucide-react';

export const IGRelatedLinks: React.FC = () => {
  const relatedLinks = [
    {
      title: 'Apa Itu Unicode?',
      path: '/blog/apa-itu-unicode',
      desc: 'Panduan lengkap standar Unicode dan cara kerja generator font.',
      icon: <BookOpen className="w-5 h-5 text-purple-400" />,
    },
    {
      title: 'Cara Membuat Nama Keren',
      path: '/blog/cara-membuat-nama-keren',
      desc: 'Panduan merancang nickname estetik untuk game & sosmed.',
    ...
    {
      title: 'Generator Font Instagram Utama',
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {relatedLinks.map((item, idx) => (
            ...
            >
              <Link
                href={item.path}
                className="bg-slate-950/80 border border-slate-800 hover:border-pink-500/50 rounded-2xl p-4 flex flex-col justify-between group transition-all block h-full hover:-translate-y-0.5"
              >
                <div>
                  ...
                  <h4 className="text-sm font-bold text-slate-100 group-hover:text-pink-300 transition-colors mb-1">
                    {item.title}
                  </h4>
