...
interface GothicExamplesProps {
  onCopy: (msg: string) => void;
}

export function GothicExamples({ onCopy }: GothicExamplesProps) {
  const [copiedIndex, setCopiedIndex] = useState<number | null>(null);

  const rawExamples = [
    { plain: 'Dark King', styleId: 'gothic-bold' },
    { plain: 'Shadow', styleId: 'fraktur-regular' },
    { plain: 'Night Wolf', styleId: 'gothic-wings' },
    { plain: 'Demon', styleId: 'dark-gothic-brackets' },
    { plain: 'Knight', styleId: 'blackletter-swords' },
    { plain: 'Empire', styleId: 'royal-gothic-crown' },
    { plain: 'Dragon', styleId: 'gothic-cross-clan' },
    { plain: 'Legend', styleId: 'gothic-skull-deco' },
    { plain: 'Royal', styleId: 'gothic-japanese-brackets' },
    { plain: 'Ghost', styleId: 'gothic-glitch' },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto">
          ...
          <h2 className="mt-4 text-3xl sm:text-5xl font-extrabold text-slate-900 dark:text-white font-poppins">
            Contoh Teks Gothic Siap Pakai
          </h2>
          <p className="mt-4 text-base sm:text-lg text-slate-600 dark:text-slate-400">
            Klik tombol salin di samping kata favorit Anda untuk menggunakannya secara instan.
          </p>
        </div>

        <div className="mt-14 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          {rawExamples.map((ex, idx) => {
            const gothicStyle = GOTHIC_FONT_STYLES.find((s) => s.id === ex.styleId) || GOTHIC_FONT_STYLES[0];
            const converted = gothicStyle.transform(ex.plain);
