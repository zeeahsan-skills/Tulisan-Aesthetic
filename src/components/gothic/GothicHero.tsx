...
import { GOTHIC_FONT_STYLES } from '@/lib/unicode-engine';
...
export function GothicHero({ onCopy }: GothicHeroProps) {
  const [inputText, setInputText] = useState('Ahsan Tulisan');
  ...
  const handleCopySingle = (transformedText: string, styleId: string) => {
    ...
    setCopiedId(styleId);
    onCopy('Font Gothic berhasil disalin!');
    setTimeout(() => setCopiedId(null), 2000);
  };

  const handleCopyAll = () => {
    const allFormatted = GOTHIC_FONT_STYLES
      .map((s) => `${s.name}:\n${s.transform(inputText || 'Ahsan Tulisan')}`)
      .join('\n\n');
    navigator.clipboard.writeText(allFormatted);
    onCopy('Semua gaya Gothic berhasil disalin!');
  };

  const handleRandomize = () => {
    const randomGothicPhrases = [
      'Dark King',
      'Shadow Knight',
      'Night Wolf',
      'Demon Hunter',
    ...
    const randomText = randomGothicPhrases[Math.floor(Math.random() * randomGothicPhrases.length)];
  ...
  const handleShare = (text: string, title: string) => {
    if (navigator.share) {
      navigator
        .share({
          title: `Gothic Font: ${title}`,
          text: text,
          url: window.location.href,
        })
        .catch(() => {});
    } else {
      navigator.clipboard.writeText(`${title}: ${text}\nGenerated via https://tulisanaesthetic.id/font/gothic-fraktur`);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-purple-500/10 via-pink-500/10 to-purple-500/10 border border-purple-500/20 text-xs sm:text-sm font-semibold text-purple-700 dark:text-purple-300 backdrop-blur-md shadow-sm">
            ...
            <span>Blackletter & Fraktur Generator 2026</span>
          </span>
        </motion.div>
        ...
        >
          ...
          <p className="mt-6 text-lg sm:text-xl text-slate-600 dark:text-slate-300 max-w-2xl mx-auto leading-relaxed">
            Convert normal text into Gothic, Fraktur, Medieval and Old English Unicode fonts instantly for gaming nicknames, Discord roles, and social media.
          </p>
        </motion.div>
          ...
          <div className="relative rounded-3xl p-6 sm:p-8 bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl border border-slate-200/80 dark:border-purple-900/40 shadow-2xl shadow-purple-500/10">
            ...
            <div className="relative">
              <textarea
                ...
                placeholder="Tulis teks Gothic di sini..."
                rows={3}
        ...
        <div className="mt-12 max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-4">
          {GOTHIC_FONT_STYLES.map((style, idx) => {
            const transformed = style.transform(inputText || 'Ahsan Tulisan');
            ...
            return (
              ...
              >
                ...
                {/* Card Action Row: Preview, Copy, Favorite, Share */}
