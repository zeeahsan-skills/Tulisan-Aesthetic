...
import { HEART_FAQS } from '@/lib/heart-symbol-faqs';

export const HeartFAQ: React.FC = () => {
  ...
  const faqSchema = {
    ...
    '@type': 'FAQPage',
    mainEntity: HEART_FAQS.map((item) => ({
      '@type': 'Question',
      name: item.q,
      acceptedAnswer: {
        '@type': 'Answer',
    ...
    <section className="py-16 bg-slate-950 text-white relative border-t border-slate-800">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        <div className="text-center max-w-3xl mx-auto mb-12">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-pink-500/10 border border-pink-500/20 text-pink-400 text-xs font-semibold mb-4">
            <HelpCircle className="w-4 h-4" />
            <span>Pertanyaan Umum</span>
          </div>
          <h2 className="text-2xl sm:text-4xl font-extrabold mb-3">
            FAQ Simbol Hati (Heart Symbols)
          </h2>
          <p className="text-slate-400 text-sm sm:text-base">
            Jawaban lengkap seputar cara menyalin simbol hati Unicode, emoji hati, border estetik, dan kompatibilitas perangkat.
          </p>
        </div>

        {/* Accordions */}
        <div className="space-y-3">
          {HEART_FAQS.map((faq, idx) => {
