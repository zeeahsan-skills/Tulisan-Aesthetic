...
import { ArrowRight, Sparkles, Heart, Star, ArrowRightLeft, Smile, Type } from 'lucide-react';

export const EmojiRelatedPages: React.FC = () => {
  const links = [
    {
    ...
    },
    {
      ...
      path: '/simbol/panah',
      desc: 'Koleksi simbol panah ➜ ➤ ➔ 🏹 untuk petunjuk arah bio.',
      icon: <ArrowRightLeft className="w-5 h-5 text-cyan-400" />,
    },
    {
      title: 'Kaomoji (Emoticon Teks)',
      path: '/simbol',
      desc: 'Koleksi emotikon teks jepang kaomoji estetik.',
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {links.map((item, idx) => (
            ...
            >
              <Link
                href={item.path}
                className="bg-slate-950/80 border border-slate-800 hover:border-amber-500/50 rounded-2xl p-5 shadow-lg flex flex-col justify-between group transition-all block h-full hover:-translate-y-1"
              >
                <div>
                  ...
                  <h3 className="text-base font-bold text-slate-100 group-hover:text-amber-300 transition-colors mb-1.5">
                    {item.title}
                  </h3>
