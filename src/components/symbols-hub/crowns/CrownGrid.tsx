...
import { CROWN_SYMBOLS_DATA, CrownSymbolItem } from '@/lib/crown-symbols';

interface CrownGridProps {
  searchQuery: string;
  activeCategory: string;
  onCopy: (msg: string) => void;
}

export const CrownGrid: React.FC<CrownGridProps> = ({
  searchQuery,
  activeCategory,
  onCopy,
}) => {
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem('mahkota_fav_symbols');
  ...
  const toggleFavorite = (symbol: string) => {
    ...
    try {
      localStorage.setItem('mahkota_fav_symbols', JSON.stringify(updated));
    } catch {
      // Ignore
    }
  };
  ...
  const handleCopy = (symbol: string) => {
    ...
    onCopy(`Simbol mahkota "${symbol}" berhasil disalin!`);
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `Simbol Mahkota ${name}`,
          text: `Salin simbol mahkota ${symbol} (${name}): ${window.location.href}`,
      ...
      navigator.clipboard.writeText(symbol);
      onCopy(`Simbol mahkota ${symbol} disalin ke clipboard!`);
    }
  };

  // Filter items based on category and search query
  const filteredItems = CROWN_SYMBOLS_DATA.filter((item) => {
    if (activeCategory !== 'All Crowns' && item.category !== activeCategory) {
      return false;
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between mb-8 pb-3 border-b border-slate-800">
          <div className="text-sm font-semibold text-slate-400">
            Menampilkan <span className="text-amber-400 font-bold">{filteredItems.length}</span> Simbol Mahkota & Kekaisaran
          </div>
          {activeCategory !== 'All Crowns' && (
        ...
        {filteredItems.length === 0 ? (
          <div className="text-center py-16 bg-slate-900/40 rounded-2xl border border-slate-800">
            <p className="text-slate-400 text-sm">Tidak ada simbol mahkota yang cocok dengan kriteria pencarian Anda.</p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  ...
                  <div
                    ...
                    className="my-2 py-3 bg-slate-950 rounded-xl border border-slate-800/80 flex items-center justify-center text-3xl sm:text-4xl font-mono text-amber-200 cursor-pointer hover:bg-amber-950/40 hover:border-amber-500/40 transition-all select-all shadow-inner min-h-[70px]"
