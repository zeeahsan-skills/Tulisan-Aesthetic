...
import { Crown, Search, Copy, Check } from 'lucide-react';
...
export const CrownHero: React.FC<CrownHeroProps> = ({
  searchQuery,
  setSearchQuery,
  onCopy,
}) => {
  ...
  const handleQuickCopy = () => {
    navigator.clipboard.writeText('♛ ♔ 👑 亗 ⚜️');
    setCopiedQuick(true);
    onCopy('Kombinasi mahkota populer berhasil disalin!');
    setTimeout(() => setCopiedQuick(false), 2000);
  };

  return (
    <section className="relative pt-12 pb-16 md:pt-16 md:pb-24 overflow-hidden bg-gradient-to-b from-slate-900 via-amber-950/40 to-slate-950 text-white">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 relative z-10 text-center">
        ...
        <motion.div
          ...
          className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-gradient-to-r from-amber-500/10 via-yellow-500/15 to-amber-500/10 border border-amber-500/30 text-amber-300 text-xs sm:text-sm font-medium mb-6 backdrop-blur-sm"
        >
          <Crown className="w-4 h-4 text-amber-400 fill-amber-400 animate-pulse" />
          <span>Unicode Crown & Royalty Symbols Collection 2026</span>
        </motion.div>
        ...
        >
          <div className="relative">
            <input
              ...
              placeholder="Cari simbol mahkota (contoh: king, queen, chess, vip, royal, crown)..."
