...
import { FLOWER_SYMBOLS_DATA, FlowerSymbolItem } from '@/lib/flower-symbols';
...
export const FlowerGrid: React.FC<FlowerGridProps> = ({
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem('bunga_fav_symbols');
  ...
  const toggleFavorite = (symbol: string) => {
    ...
    try {
      localStorage.setItem('bunga_fav_symbols', JSON.stringify(updated));
    } catch {
      // Ignore
    }
  };
  ...
  const handleCopy = (symbol: string) => {
    ...
    onCopy(`Simbol bunga "${symbol}" berhasil disalin!`);
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `Simbol Bunga ${name}`,
          text: `Salin simbol bunga ${symbol} (${name}): ${window.location.href}`,
      ...
      navigator.clipboard.writeText(symbol);
      onCopy(`Simbol bunga ${symbol} disalin ke clipboard!`);
    }
  };

  // Filter items based on category and search query
  const filteredItems = FLOWER_SYMBOLS_DATA.filter((item) => {
    if (activeCategory !== 'All Flowers' && item.category !== activeCategory) {
      return false;
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between mb-8 pb-3 border-b border-slate-800">
          <div className="text-sm font-semibold text-slate-400">
            Menampilkan <span className="text-rose-400 font-bold">{filteredItems.length}</span> Simbol Bunga & Flora
          </div>
          {activeCategory !== 'All Flowers' && (
            <span className="text-xs font-bold text-rose-300 bg-rose-500/10 px-3 py-1 rounded-full border border-rose-500/30">
              {activeCategory}
            </span>
          )}
        </div>
        ...
        {filteredItems.length === 0 ? (
          <div className="text-center py-16 bg-slate-900/40 rounded-2xl border border-slate-800">
            <p className="text-slate-400 text-sm">Tidak ada simbol bunga yang cocok dengan kriteria pencarian Anda.</p>
          </div>
        ) : (
            ...
            {filteredItems.map((item, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className="bg-slate-900/90 border border-slate-800 hover:border-rose-500/50 rounded-2xl p-4 shadow-lg flex flex-col justify-between group transition-all"
                >
                  {/* Card Header (Category Badge & Favorite) */}
                  <div className="flex items-center justify-between text-xs mb-2">
                    <span className="text-[10px] font-semibold text-rose-400 bg-rose-500/10 px-2 py-0.5 rounded border border-rose-500/20 truncate max-w-[90px]">
                      {item.category}
                    </span>
                    <button
                  ...
                  <div
                    ...
                    className="my-2 py-3 bg-slate-950 rounded-xl border border-slate-800/80 flex items-center justify-center text-3xl sm:text-4xl font-mono text-rose-200 cursor-pointer hover:bg-rose-950/40 hover:border-rose-500/40 transition-all select-all shadow-inner min-h-[70px]"
