...
import { STAR_SYMBOLS_DATA, StarSymbolItem } from '@/lib/star-symbols';
...
export const StarGrid: React.FC<StarGridProps> = ({
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem('bintang_fav_symbols');
  ...
  const toggleFavorite = (symbol: string) => {
    ...
    try {
      localStorage.setItem('bintang_fav_symbols', JSON.stringify(updated));
    } catch {
      // Ignore
    }
  };
  ...
  const handleCopy = (symbol: string) => {
    ...
    onCopy(`Simbol bintang "${symbol}" berhasil disalin!`);
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `Simbol Bintang ${name}`,
          text: `Salin simbol bintang ${symbol} (${name}): ${window.location.href}`,
      ...
      navigator.clipboard.writeText(symbol);
      onCopy(`Simbol bintang ${symbol} disalin ke clipboard!`);
    }
  };

  // Filter items based on category and search query
  const filteredItems = STAR_SYMBOLS_DATA.filter((item) => {
    if (activeCategory !== 'All Stars' && item.category !== activeCategory) {
      return false;
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between mb-8 pb-3 border-b border-slate-800">
          <div className="text-sm font-semibold text-slate-400">
            Menampilkan <span className="text-amber-400 font-bold">{filteredItems.length}</span> Simbol Bintang
          </div>
          {activeCategory !== 'All Stars' && (
        ...
        {filteredItems.length === 0 ? (
          <div className="text-center py-16 bg-slate-900/40 rounded-2xl border border-slate-800">
            <p className="text-slate-400 text-sm">Tidak ada simbol bintang yang cocok dengan kriteria pencarian Anda.</p>
          </div>
        ) : (
