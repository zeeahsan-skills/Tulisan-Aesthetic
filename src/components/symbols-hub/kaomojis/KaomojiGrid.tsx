...
import { KAOMOJI_SYMBOLS_DATA, KaomojiSymbolItem } from '@/lib/kaomoji-symbols';
...
export const KaomojiGrid: React.FC<KaomojiGridProps> = ({
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem('kaomoji_fav_symbols');
  ...
  const toggleFavorite = (symbol: string) => {
    ...
    try {
      localStorage.setItem('kaomoji_fav_symbols', JSON.stringify(updated));
    } catch {
      // Ignore
    }
  };
  ...
  const handleCopy = (symbol: string) => {
    ...
    onCopy(`Kaomoji "${symbol}" berhasil disalin!`);
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `Kaomoji ${name}`,
          text: `Salin Kaomoji ${symbol} (${name}): ${window.location.href}`,
      ...
      navigator.clipboard.writeText(symbol);
      onCopy(`Kaomoji ${symbol} disalin ke clipboard!`);
    }
  };

  // Filter items based on category and search query
  const filteredItems = KAOMOJI_SYMBOLS_DATA.filter((item) => {
    if (activeCategory !== 'All Kaomoji' && item.category !== activeCategory) {
      return false;
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between mb-8 pb-3 border-b border-slate-800">
          <div className="text-sm font-semibold text-slate-400">
            Menampilkan <span className="text-teal-400 font-bold">{filteredItems.length}</span> Kaomoji Text Faces
          </div>
          {activeCategory !== 'All Kaomoji' && (
        ...
        {filteredItems.length === 0 ? (
          <div className="text-center py-16 bg-slate-900/40 rounded-2xl border border-slate-800">
            <p className="text-slate-400 text-sm">Tidak ada Kaomoji yang cocok dengan kriteria pencarian Anda.</p>
          </div>
        ) : (
            ...
            {filteredItems.map((item, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className="bg-slate-900/90 border border-slate-800 hover:border-teal-500/50 rounded-2xl p-4 shadow-lg flex flex-col justify-between group transition-all"
                >
                  {/* Card Header (Category Badge & Favorite) */}
                  <div className="flex items-center justify-between text-xs mb-2">
                    <span className="text-[10px] font-semibold text-teal-400 bg-teal-500/10 px-2 py-0.5 rounded border border-teal-500/20 truncate max-w-[100px]">
                      {item.category}
                    </span>
                    <button
                  ...
                  <div
                    ...
                    className="my-2 py-4 px-3 bg-slate-950 rounded-xl border border-slate-800/80 flex items-center justify-center text-xl sm:text-2xl font-mono text-teal-200 cursor-pointer hover:bg-teal-950/40 hover:border-teal-500/40 transition-all select-all shadow-inner min-h-[75px] text-center"
