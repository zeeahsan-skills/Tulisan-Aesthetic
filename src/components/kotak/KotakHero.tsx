...
import { Sparkles, Copy, Shuffle, Trash2, Wand2, Check, Heart, Share2, Square } from 'lucide-react';
import { SQUARE_FONT_STYLES } from '@/lib/unicode-engine';
...
export function KotakHero({ onCopy }: KotakHeroProps) {
  ...
  const handleCopySingle = (transformedText: string, styleId: string) => {
    ...
    setCopiedId(styleId);
    onCopy('Font Kotak (Square) berhasil disalin!');
    setTimeout(() => setCopiedId(null), 2000);
  };

  const handleCopyAll = () => {
    const allFormatted = SQUARE_FONT_STYLES
      .map((s) => `${s.name}:\n${s.transform(inputText || 'Ahsan Stylish')}`)
      .join('\n\n');
    navigator.clipboard.writeText(allFormatted);
    onCopy('Semua gaya Font Kotak berhasil disalin!');
  };

  const handleRandomize = () => {
    const randomSquarePhrases = [
      'Gamer VIP',
      'Pro Legend',
      'King Winner',
      'Queen Style',
    ...
    const randomText = randomSquarePhrases[Math.floor(Math.random() * randomSquarePhrases.length)];
  ...
  const handleShare = (text: string, title: string) => {
    if (navigator.share) {
      navigator
        .share({
          title: `Font Kotak: ${title}`,
          text: text,
          url: window.location.href,
        })
        .catch(() => {});
    } else {
      navigator.clipboard.writeText(`${title}: ${text}\nGenerated via https://tulisanaesthetic.id/font/kotak`);
  ...
  return (
    <section id="hero" className="relative pt-12 pb-20 md:pt-20 md:pb-32 overflow-hidden">
      ...
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-gradient-to-tr from-cyan-500/20 via-blue-500/20 to-purple-500/10 rounded-full blur-3xl -z-10 pointer-events-none" />
      <div className="absolute top-1/3 right-10 w-96 h-96 bg-blue-500/15 rounded-full blur-3xl -z-10 pointer-events-none" />
      <div className="absolute bottom-10 left-10 w-96 h-96 bg-cyan-500/15 rounded-full blur-3xl -z-10 pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-cyan-500/10 via-blue-500/10 to-purple-500/10 border border-cyan-500/20 text-xs sm:text-sm font-semibold text-cyan-700 dark:text-cyan-300 backdrop-blur-md shadow-sm">
            <Square className="w-4 h-4 text-cyan-500 animate-pulse" />
            <span>Square & Boxed Unicode Font Generator 2026</span>
          </span>
        </motion.div>
        ...
        >
          ...
          <p className="mt-6 text-lg sm:text-xl text-slate-600 dark:text-slate-300 max-w-2xl mx-auto leading-relaxed">
            Convert normal text into square and boxed Unicode text instantly.
          </p>
        </motion.div>
          ...
          <div className="relative rounded-3xl p-6 sm:p-8 bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl border border-slate-200/80 dark:border-purple-900/40 shadow-2xl shadow-cyan-500/10">
            ...
            <div className="flex items-center justify-between mb-3">
              <label className="text-xs sm:text-sm font-bold uppercase tracking-wider text-slate-600 dark:text-cyan-300 flex items-center gap-2">
                <Wand2 className="w-4 h-4 text-cyan-500 dark:text-blue-400" />
                Input Teks Kotak Anda:
              </label>
              {inputText && (
                <button
            ...
            <div className="relative">
              <textarea
                ...
                placeholder="Tulis teks kotak di sini..."
                rows={3}
            ...
            <div className="mt-5 flex flex-wrap items-center gap-3">
              ...
              >
                <Copy className="w-4 h-4 text-cyan-500 dark:text-blue-400" />
                Salin Semua
              </button>
        ...
        <div className="mt-12 max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-4">
          {SQUARE_FONT_STYLES.map((style, idx) => {
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    ...
                    <button
                      ...
                      title="Bagikan"
                      className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-cyan-600 dark:hover:text-blue-400 transition"
                    >
