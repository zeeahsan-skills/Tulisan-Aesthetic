...
import { Sparkles, Copy, Shuffle, Trash2, Wand2, Check, Heart, Share2, Skull } from 'lucide-react';
import { SCARY_FONT_STYLES } from '@/lib/unicode-engine';
...
export function MenakutkanHero({ onCopy }: MenakutkanHeroProps) {
  ...
  const handleCopySingle = (transformedText: string, styleId: string) => {
    ...
    setCopiedId(styleId);
    onCopy('Font Menakutkan / Glitch berhasil disalin!');
    setTimeout(() => setCopiedId(null), 2000);
  };

  const handleCopyAll = () => {
    const allFormatted = SCARY_FONT_STYLES
      .map((s) => `${s.name}:\n${s.transform(inputText || 'Ahsan Stylish')}`)
      .join('\n\n');
    navigator.clipboard.writeText(allFormatted);
    onCopy('Semua gaya Font Menakutkan berhasil disalin!');
  };

  const handleRandomize = () => {
    const randomScaryPhrases = [
      'Ghost Rider',
      'Zombie King',
      'Nightmare Demon',
      'Monster Shadow',
    ...
    const randomText = randomScaryPhrases[Math.floor(Math.random() * randomScaryPhrases.length)];
  ...
  const handleShare = (text: string, title: string) => {
    if (navigator.share) {
      navigator
        .share({
          title: `Font Menakutkan: ${title}`,
          text: text,
          url: window.location.href,
        })
        .catch(() => {});
    } else {
      navigator.clipboard.writeText(`${title}: ${text}\nGenerated via https://tulisanaesthetic.id/font/menakutkan`);
  ...
  return (
    <section id="hero" className="relative pt-12 pb-20 md:pt-20 md:pb-32 overflow-hidden">
      ...
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-gradient-to-tr from-rose-600/20 via-purple-600/20 to-slate-900/40 rounded-full blur-3xl -z-10 pointer-events-none" />
      <div className="absolute top-1/3 right-10 w-96 h-96 bg-red-600/15 rounded-full blur-3xl -z-10 pointer-events-none" />
      <div className="absolute bottom-10 left-10 w-96 h-96 bg-purple-900/20 rounded-full blur-3xl -z-10 pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-rose-500/10 via-purple-500/10 to-slate-900/10 border border-rose-500/20 text-xs sm:text-sm font-semibold text-rose-600 dark:text-rose-400 backdrop-blur-md shadow-sm">
            <Skull className="w-4 h-4 text-rose-500 animate-pulse" />
            <span>Scary, Creepy & Zalgo Text Generator 2026</span>
          </span>
        </motion.div>
        ...
        >
          <h1 className="text-4xl sm:text-6xl lg:text-7xl font-extrabold tracking-tight text-slate-900 dark:text-white font-poppins leading-[1.15]">
            Font Menakutkan /{' '}
            <span className="bg-gradient-to-r from-rose-600 via-purple-500 to-red-500 bg-clip-text text-transparent">
              Zalgo Glitch
            </span>
          </h1>
          <p className="mt-6 text-lg sm:text-xl text-slate-600 dark:text-slate-300 max-w-2xl mx-auto leading-relaxed">
            Convert normal text into scary, creepy, glitch, and Zalgo-style Unicode text instantly.
          </p>
        </motion.div>
        ...
        >
          <div className="relative rounded-3xl p-6 sm:p-8 bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl border border-slate-200/80 dark:border-purple-900/40 shadow-2xl shadow-rose-500/10">
            
            <div className="flex items-center justify-between mb-3">
              <label className="text-xs sm:text-sm font-bold uppercase tracking-wider text-slate-600 dark:text-rose-300 flex items-center gap-2">
                <Wand2 className="w-4 h-4 text-rose-600 dark:text-rose-400" />
                Input Teks Menakutkan Anda:
              </label>
              {inputText && (
                <button
            ...
            <div className="relative">
              <textarea
                ...
                placeholder="Tulis teks seram / glitch di sini..."
                rows={3}
                className="w-full px-5 py-4 text-lg sm:text-2xl font-medium rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-purple-900/50 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-rose-500/50 focus:border-rose-500 transition-all resize-none shadow-inner"
              />
            </div>
        ...
        <div className="mt-12 max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-4">
          {SCARY_FONT_STYLES.map((style, idx) => {
              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-rose-500/10 text-rose-600 dark:text-rose-300 font-medium">
                      Unicode Zalgo
                    </span>
                  </div>
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    ...
                    <button
                      ...
                      title="Bagikan"
                      className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-rose-600 dark:hover:text-pink-400 transition"
                    >
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all duration-200 ${
                      isCopied
                        ...
                        : 'bg-rose-50 dark:bg-rose-950/50 hover:bg-rose-600 hover:text-white text-rose-600 dark:text-rose-300 dark:hover:bg-rose-600 border border-rose-200 dark:border-rose-800/50'
                    }`}
                  >
                    {isCopied ? (
                      <>
