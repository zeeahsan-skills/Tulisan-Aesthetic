...
import { Sparkles, Copy, Shuffle, Trash2, Wand2, Check, Heart, Share2, Strikethrough } from 'lucide-react';
import { STRIKETHROUGH_FONT_STYLES } from '@/lib/unicode-engine';
...
export function GarisHero({ onCopy }: GarisHeroProps) {
  ...
  const handleCopySingle = (transformedText: string, styleId: string) => {
    ...
    setCopiedId(styleId);
    onCopy('Font Garis Coret berhasil disalin!');
    setTimeout(() => setCopiedId(null), 2000);
  };

  const handleCopyAll = () => {
    const allFormatted = STRIKETHROUGH_FONT_STYLES
      .map((s) => `${s.name}:\n${s.transform(inputText || 'Ahsan Stylish')}`)
      .join('\n\n');
    navigator.clipboard.writeText(allFormatted);
    onCopy('Semua gaya Font Garis berhasil disalin!');
  };

  const handleRandomize = () => {
    const randomGarisPhrases = [
      'Cancelled',
      'Sold Out',
      'Finished',
      'Expired',
    ...
    const randomText = randomGarisPhrases[Math.floor(Math.random() * randomGarisPhrases.length)];
  ...
  const handleShare = (text: string, title: string) => {
    if (navigator.share) {
      navigator
        .share({
          title: `Font Garis: ${title}`,
          text: text,
          url: window.location.href,
        })
        .catch(() => {});
    } else {
      navigator.clipboard.writeText(`${title}: ${text}\nGenerated via https://tulisanaesthetic.id/font/garis`);
  ...
  return (
    <section id="hero" className="relative pt-12 pb-20 md:pt-20 md:pb-32 overflow-hidden">
      ...
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-gradient-to-tr from-indigo-600/20 via-pink-500/20 to-amber-500/10 rounded-full blur-3xl -z-10 pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-indigo-500/10 via-pink-500/10 to-rose-500/10 border border-indigo-500/20 text-xs sm:text-sm font-semibold text-indigo-700 dark:text-indigo-300 backdrop-blur-md shadow-sm">
            <Strikethrough className="w-4 h-4 text-indigo-500 animate-pulse" />
            <span>Strikethrough & Crossed Out Text Generator 2026</span>
          </span>
        </motion.div>
        ...
        >
          ...
          <p className="mt-6 text-lg sm:text-xl text-slate-600 dark:text-slate-300 max-w-2xl mx-auto leading-relaxed">
            Convert normal text into Unicode Strikethrough text instantly for social media, messaging apps, and gaming.
          </p>
        </motion.div>
        ...
        >
          <div className="relative rounded-3xl p-6 sm:p-8 bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl border border-slate-200/80 dark:border-purple-900/40 shadow-2xl shadow-indigo-500/10">
            
            <div className="flex items-center justify-between mb-3">
              <label className="text-xs sm:text-sm font-bold uppercase tracking-wider text-slate-600 dark:text-indigo-300 flex items-center gap-2">
                <Wand2 className="w-4 h-4 text-indigo-600 dark:text-pink-400" />
                Input Teks Garis Anda:
              </label>
              {inputText && (
                <button
            ...
            <div className="relative">
              <textarea
                ...
                placeholder="Tulis teks garis coret di sini..."
                rows={3}
            ...
            <div className="mt-5 flex flex-wrap items-center gap-3">
              ...
              >
                <Copy className="w-4 h-4 text-indigo-600 dark:text-pink-400" />
                Salin Semua
              </button>
        ...
        <div className="mt-12 max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-4">
          {STRIKETHROUGH_FONT_STYLES.map((style, idx) => {
              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-indigo-500/10 text-indigo-600 dark:text-indigo-300 font-medium">
                      Unicode Strike
                    </span>
                  </div>
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    ...
                    <button
                      ...
                      title="Bagikan"
                      className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-pink-400 transition"
                    >
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all duration-200 ${
                      isCopied
                        ...
                        : 'bg-indigo-50 dark:bg-indigo-950/50 hover:bg-indigo-600 hover:text-white text-indigo-600 dark:text-indigo-300 dark:hover:bg-indigo-600 border border-indigo-200 dark:border-indigo-800/50'
                    }`}
                  >
                    {isCopied ? (
                      <>
