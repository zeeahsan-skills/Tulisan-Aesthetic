...
import { Sparkles, Copy, Shuffle, Trash2, Wand2, Check, Heart, Share2 } from 'lucide-react';
...
export function BerkilauHero({ onCopy }: BerkilauHeroProps) {
  ...
  const handleCopySingle = (transformedText: string, styleId: string) => {
    ...
    setCopiedId(styleId);
    onCopy('Font Berkilau berhasil disalin!');
    setTimeout(() => setCopiedId(null), 2000);
  };

  const handleCopyAll = () => {
    const allFormatted = SPARKLY_FONT_STYLES
      .map((s) => `${s.name}:\n${s.transform(inputText || 'Ahsan Stylish')}`)
      .join('\n\n');
    navigator.clipboard.writeText(allFormatted);
    onCopy('Semua gaya Font Berkilau berhasil disalin!');
  };

  const handleRandomize = () => {
    const randomSparklyPhrases = [
      'Princess Queen',
      'Luxury Diamond',
      'Magic Dream',
      'Galaxy Star',
    ...
    const randomText = randomSparklyPhrases[Math.floor(Math.random() * randomSparklyPhrases.length)];
  ...
  const handleShare = (text: string, title: string) => {
    if (navigator.share) {
      navigator
        .share({
          title: `Font Berkilau: ${title}`,
          text: text,
          url: window.location.href,
        })
        .catch(() => {});
    } else {
      navigator.clipboard.writeText(`${title}: ${text}\nGenerated via https://tulisanaesthetic.id/font/berkilau`);
  ...
  return (
    <section id="hero" className="relative pt-12 pb-20 md:pt-20 md:pb-32 overflow-hidden">
      ...
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-gradient-to-tr from-amber-400/20 via-pink-500/20 to-purple-500/10 rounded-full blur-3xl -z-10 pointer-events-none" />
      <div className="absolute top-1/3 right-10 w-96 h-96 bg-amber-500/15 rounded-full blur-3xl -z-10 pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-amber-500/10 via-pink-500/10 to-purple-500/10 border border-amber-500/20 text-xs sm:text-sm font-semibold text-amber-700 dark:text-amber-300 backdrop-blur-md shadow-sm">
            <Sparkles className="w-4 h-4 text-amber-500 animate-spin" />
            <span>Sparkly, Fancy & Decorative Unicode Generator 2026</span>
          </span>
        </motion.div>
        ...
        >
          ...
          <p className="mt-6 text-lg sm:text-xl text-slate-600 dark:text-slate-300 max-w-2xl mx-auto leading-relaxed">
            Convert normal text into sparkling, decorative, and aesthetic Unicode text instantly.
          </p>
        </motion.div>
        ...
        >
          <div className="relative rounded-3xl p-6 sm:p-8 bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl border border-slate-200/80 dark:border-purple-900/40 shadow-2xl shadow-amber-500/10">
            
            <div className="flex items-center justify-between mb-3">
              <label className="text-xs sm:text-sm font-bold uppercase tracking-wider text-slate-600 dark:text-amber-300 flex items-center gap-2">
                <Wand2 className="w-4 h-4 text-amber-500 dark:text-pink-400" />
                Input Teks Berkilau Anda:
              </label>
              {inputText && (
                <button
            ...
            <div className="relative">
              <textarea
                ...
                placeholder="Tulis teks berkilau di sini..."
                rows={3}
                className="w-full px-5 py-4 text-lg sm:text-2xl font-medium rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-purple-900/50 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 transition-all resize-none shadow-inner"
              />
            </div>
            ...
            <div className="mt-5 flex flex-wrap items-center gap-3">
              ...
              >
                <Copy className="w-4 h-4 text-amber-500 dark:text-pink-400" />
                Salin Semua
              </button>
        ...
        <div className="mt-12 max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-4">
          {SPARKLY_FONT_STYLES.map((style, idx) => {
              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-600 dark:text-amber-300 font-medium">
                      Unicode Sparkle
                    </span>
                  </div>
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    ...
                    <button
                      ...
                      title="Bagikan"
                      className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-amber-600 dark:hover:text-pink-400 transition"
                    >
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all duration-200 ${
                      isCopied
                        ...
                        : 'bg-amber-50 dark:bg-amber-950/50 hover:bg-amber-600 hover:text-white text-amber-600 dark:text-amber-300 dark:hover:bg-amber-600 border border-amber-200 dark:border-amber-800/50'
                    }`}
                  >
                    {isCopied ? (
                      <>
